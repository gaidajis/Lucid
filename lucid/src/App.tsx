...
function App() {
  ...
  // Memoize filtering and grouping into a single pass to prevent unnecessary re-renders
  // and O(N * Tiers) sequential filtering
  const itemsByTier = useMemo(() => {
    ...
    items.forEach((item: LucidItem) => {
      if (activeFilter.modality !== 'all' && item.modality !== activeFilter.modality) return;
      if (activeFilter.budget !== 'all' && item.context.budget !== activeFilter.budget) return;

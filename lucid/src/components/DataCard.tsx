...
export const DataCard: React.FC<DataCardProps> = ({ item, editMode, onEdit, onDelete, onView }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-card-light dark:bg-card border border-border-light dark:border-border rounded-lg overflow-hidden transition-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black dark:focus-visible:ring-white ${
        !editMode ? 'hover:shadow-[0_4px_16px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_4px_16px_rgba(0,0,0,0.4)] cursor-pointer' : ''
      }`}
      onClick={() => !editMode && onView(item)}
      role={!editMode ? "button" : undefined}
      tabIndex={!editMode ? 0 : undefined}
      aria-label={!editMode ? `View details for ${item.title}` : undefined}
      onKeyDown={(e) => {
        if (!editMode) {
      ...
      {editMode && (
        <div className="absolute top-2 right-2 z-10 flex gap-2">
          <button
            ...
            className="p-2 bg-white/90 dark:bg-gray-800/90 rounded-full hover:bg-white dark:hover:bg-gray-700 transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black dark:focus-visible:ring-white"
          ...
          <button
            ...
            className="p-2 bg-white/90 dark:bg-gray-800/90 rounded-full hover:bg-red-500 hover:text-white transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black dark:focus-visible:ring-white"
